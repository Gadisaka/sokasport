 ...
 * @returns {Promise<{ byApiId: Map<number, object>, pages: number, items: number, stoppedForBudget: boolean }>}
 */
async function fetchOddsPagesForDate(sportSlug, date) {
  ...
  while (page <= totalPages && page <= maxPages) {
    ...
    if (!budget.allowed) {
      ...
      console.warn(
        `[syncOddsBulk] daily odds API budget exhausted during ${sportSlug} ${date} (used=${budget.used}/${budget.budget})`,
      );
      break;
    }
  ...
  return { byApiId, pages, items, stoppedForBudget };
}
...
export default async function syncOddsBulkByDate(options = {}) {
  ...
  outer: for (const sportSlug of enabled) {
    for (const date of dates) {
      ...
      let items = 0;
      try {
        ({ byApiId, pages, items, stoppedForBudget } =
          await fetchOddsPagesForDate(sportSlug, date));
      } catch (err) {
        console.error(
      ...
      for (const fixture of missingFixtures) {
        ...
        if (!item) {
          // Upstream page set has no odds for this fixture — schedule recheck.
  ...
  console.log(
    `[syncOddsBulk] done (${label}) – pages=${totalPages}, items=${totalItems}, matched=${totalMatched}, persisted=${totalPersisted}, skippedUnknown=${totalSkippedUnknown}, emptyParse=${totalSkippedEmptyParse}, upserts=${totalUpserts}${stoppedForBudget ? " (budget-stop)" : ""}`,
  );
